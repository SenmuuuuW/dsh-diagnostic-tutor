 ...
 */
export const RUNTIME_SEMANTICS_TEXT = `Learning runtime state is explicit and user-visible: a learning goal and a diagnosis map, stored locally and exportable by the learner on request. Treat stored state as evidence to be re-checked rather than settled truth — a node reads "confirmed" only because recorded evidence supported it, and a node without such evidence stays "unconfirmed". The map is diagnosis-driven and reversible: grow it as diagnosis reveals a prerequisite or a blocker, never as a pre-planned syllabus. It records no scores and no completion percentages. The runtime also holds a **learning focus**: the node the learner is working on now. A teaching turn that reaches a conclusion ends by recording where the learner goes next (udt_decide_next) — the runtime stores that decision and shows it with the reason, and **nothing moves until the learner chooses to continue**, so a turn that judged an answer is not finished until the next step is recorded.`
